...
module Vatcalc    
  class Base
    ...
    def initialize
      @percentage_hash = Util::PercentageHash.new
    end
    ...
    def <<(arg)
      b = BaseObject.convert(arg)
      @currency = b.currency
      collection << b
      self[b.percentage] ? self[b.percentage] += b : self[b.percentage] = b
      return self
    end

    alias :insert :<<
    delegate :[],:[]=, to: :@percentage_hash
    ...
    BaseObject::GNV.each do |m|
      define_method(m) do 
        @percentage_hash.values.inject(Money.new(0,@currency || Vatcalc.currency)){|sum,it| sum += it.send(m)}
      end
    end
